...
from mars.base.utils.errors import ReplacementTransactionUnderpriced, InsufficientBalance, alwaysFail, TransactionPending, UnknownTransaction, NetworkError, TransactionDisallowed
...
class Transaction(object):
    ...
    def fetch_transaction(self, tx, round=None, api=None, *args, **kwargs):
        ...
        self.require(
            self.least_balance > baseCurrency["balance"], InsufficientBalance(baseCurrency, f"at lest need : {self.least_balance}")
        )
        self.require(
            self.endurable_gasFee < self.gas_fee, TransactionDisallowed(self.gas_fee, f"gasfee :{self.gas_fee} should be less then endurable_gasFee :{self.endurable_gasFee}")
        )
    ...
    def fetch_add_liquidity(self, tx: Optional[AttributeDict] = None, tx_receipt = None, api=False, *args, **kwargs) -> Dict[str, Any]:
        """
        Info
        ----------
        ...
        txDict = {
            ...
            "input": self.input,
            "gas_fee": tx_receipt["gasUsed"] * tx_receipt["effectiveGasPrice"] / 10**18 + self.additionalGasFee,
            "tx_scope": f"{self.chains['mainnet']['block_scope']}/tx/{tx_receipt['transactionHash'].hex()}"
        }

        return txDict
    
    def fetch_remove_liquidity(self, tx: Optional[AttributeDict] = None, tx_receipt = None, api=False, *args, **kwargs) -> Dict[str, Any]:
        """
        Info
        ----------
        Fetches the details of a swap transaction.
    ...
    def fetch_fusion_swap(self, tx: Optional[AttributeDict] = None):
        ...
        amountOut = self.to_value(int(tx["order"]["takingAmount"]), self.decimals(self.tokenBsymbol))
    ...
    def fetch_swap(self, tx: Optional[AttributeDict] = None, tx_receipt = None, api=False) -> Dict[str, Any]:
        """
        Info
        ----------
        Fetches the details of a swap transaction.
        ...
        if not api :
            ...
            quote_total_tx = self.create_request(params, 'v3', f'{self.chains["mainnet"]["chain_id"]}', 'getTransaction')
