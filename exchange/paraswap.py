...
class Paraswap(Exchange):
    ...
    def token_to_token(self, amountA, tokenA, tokenAaddress, tokenB, tokenBaddress, accountAddress, routerAddress, slippage, priceRoute):
        ...
        if self.proxy :
            ...
            quote_tx = self.create_request(self.api_url, params, 'transactions', f"{self.chains['mainnet']['chain_id']}", request_method='post', proxy=self.proxy)
            
        else :

            quote_tx = self.create_request(self.api_url, params, 'transactions', f"{self.chains['mainnet']['chain_id']}", request_method='post')
