...
class Openocean(Exchange):
    ...
    async def fetch_ticker(self, amountAin, tokenAsymbol, tokenBsymbol, **kwargs):
        ...
        if self.proxy :
            ...
            quote_result = self.create_request(self.api_url, params, 'v2', f"{self.chains['mainnet']['chain_id']}", 'quote', proxy=self.proxy)
            
        else :
            
            quote_result = self.create_request(self.api_url, params, 'v2', f"{self.chains['mainnet']['chain_id']}", 'quote')
            
        amountout = self.to_value(
            value=quote_result["outAmount"], exp=self.decimals(tokenBsymbol)
        )
    ...
    def token_to_token(self, tokenAaddress, amountA, tokenBaddress, accountAddress, routerAddress, slippage):
        ...
        if self.proxy :
            ...
            quote_result = self.create_request(self.api_url, params, 'v2', f"{self.chains['mainnet']['chain_id']}", 'swap', proxy=self.proxy)
            
        else :
            
            quote_result = self.create_request(self.api_url, params, 'v2', f"{self.chains['mainnet']['chain_id']}", 'swap')
