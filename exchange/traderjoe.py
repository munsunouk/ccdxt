...
from mars.base.utils.constants import curve_4batch
...
class Traderjoe(Exchange):
    ...
    async def add_liquidity(
        ...
        self.factoryContract = self.get_contract(factoryAddress, self.markets["factoryAbi"][int(isV21)])
        self.poolContract = self.get_contract(self.poolAddress, self.markets["poolAbi"][int(isV21)])
        self.routerContract = self.get_contract(routerAddress, self.markets["routerAbi"][int(isV21)])
        
        self.require(not self.fetch_available_LBPairs(tokenAaddress, tokenBaddress, self.poolAddress), ValueError("can`t find any available pairs"))
    ...
    async def remove_liquidity(
        ...
        self.require(f"{tokenAsymbol}-{tokenBsymbol}" not in self.pools, ValueError("can`t find any available pairs in mars"))
        ...
        binId_result = self.create_request(self.api_url, {}, "v1", "user", "bin-ids", self.account, "avalanche", pool["poolAddress"])
        ...
        self.require(not self.fetch_available_LBPairs(tokenAaddress, tokenBaddress, poolAddress), ValueError("can`t find any available pairs"))
    
        self.check_approve_all(
            router=self.markets["routerAddress"][int(isV21)]
        )
    ...
    def add_liquidity_parameters(
        ...
        if idSlippage == 3 :
            ...
            tokenAamounts = list(map(lambda x: int(round(x * tokenAamount)), curve_4batch['batchX']))
            tokenBamounts = list(map(lambda x: int(round(x * tokenBamount)), curve_4batch['batchY']))
            
            amountAMins = list(map(lambda x: int(round(x * (1 - amountSlippage / 100))), tokenAamounts))
            amountBMins = list(map(lambda x: int(round(x * (1 - amountSlippage / 100))), tokenBamounts))
        
        for i, bin_range in enumerate(bin_range_list) :
            
            deltaIds, distributionX, distributionY = self.get_curve_distribution_from_bin_range(activeId, bin_range, [tokenAamounts[i], tokenBamounts[i]])
            distributionX, distributionY = curve_4batch['distributionX'][i], curve_4batch['distributionY'][i]
            payload =  {
    ...
    def remove_liquidity_parameters(
        ...
        for i, ids in enumerate(ids_list) :
            ...
            for j, id in enumerate(ids) :
                ...
                totalXBalanceWithdrawn += LBTokenAmount * binReserveX / self.poolContract.functions.totalSupply(id)
                totalYBalanceWithdrawn += LBTokenAmount * binReserveY / self.poolContract.functions.totalSupply(id)
    ...
    def get_uniform_distribution_from_bin_range(self, tokenA, tokenB, active_id, bin_range, parsed_amounts):
        ...
        if bin_range[1] <= active_id and parsed_amount_a == 0:
            ...
            negative_delta_ids = [-1 * (el + 1) for el in range(active_id - bin_range[0] - 1, -1, -1)][:neg_delta]
        ...
        # Range only includes A tokens (X tokens)
            ...
            positive_delta_ids = [(el + 1) for el in range(bin_range[1] - active_id)][::-1][:pos_delta][::-1]
        ...
        # Range includes both X and Y tokens
            ...
            _distribution_x = [0]*neg_delta + [pos_pct_per_bin / 2] + [pos_pct_per_bin]*pos_delta
            _distribution_y = [neg_pct_per_bin]*neg_delta + [neg_pct_per_bin / 2] + [0]*pos_delta
    ...
    def get_curve_distribution_from_bin_range(self, active_id, bin_range, parsed_amounts):
        ...
        if bin_range[1] <= active_id and parsed_amount_a == 0:
            ...
            _distribution_y = [2 * A * math.exp(-0.5 * ((R - ind) / sigma) ** 2) for ind, _ in enumerate(delta_ids)]
        ...
        # Range only includes A tokens (X tokens)
            ...
            _distribution_x = [2 * A * math.exp(-0.5 * (ind / sigma) ** 2) for ind, _ in enumerate(delta_ids)]
        ...
        # Range includes both X and Y tokens
            ...
            _distribution_x = [0 for _ in range(neg_delta)] + [AX] + [2 * AX * math.exp(-0.5 * ((ind + 1) / sigmaX) ** 2) for ind, _ in enumerate(positive_delta_ids)]
            ...
            # Dist = 2 * A * exp(-0.5 * (r / sigma) ^ 2)
            # R is distance from 0
            _distribution_y = [2 * AY * math.exp(-0.5 * ((RY - ind) / sigmaY) ** 2) for ind, _ in enumerate(negative_delta_ids)] + [AY] + [0 for _ in range(pos_delta)]
    ...
    def fetch_available_LBPairs(self, tokenAaddress, tokenBaddress, poolAddress):
        ...
        pairs = self.factoryContract.functions.getAllLBPairs(
    ...
    def get_IdSlippage_From_PriceSlippage(self, number_bins, binStep, priceSlippage):
        ...
        result = math.floor(
            math.log(1 + priceSlippage / 100) / math.log(1 + binStep / 10000)
        )
    ...
    def token_with_token(self, payload):
        ...
        tx = self.routerContract.functions.addLiquidity(
            payload
        ).build_transaction(
            {
                "from": self.account,
    ...
    def eth_with_token(self, payload, amount):
        ...
        tx = self.routerContract.functions.addLiquidityNATIVE(
            payload
        ).build_transaction(
            {
                "from": self.account,
    ...
    def get_debank_balance(self) :
        ...
        params = {
            "user_addr" : self.account
        }
        
        wallet_balance_list =  self.create_request(debank_link, params, "portfolio", "project_list")["data"]
    ...
    def get_coinstatus_balance(self) :
        ...
        assets = self.create_request(coinstatus_link, {}, "v2", "portfolios", "defi", "tracking?portfolioId=", header=uuid_header)['protocols'][0]['investments'][0]['assets']
    ...
    def get_last_tx(self) :
        ...
        last_tx = self.create_request("https://api.snowtrace.io/", params, "api")#['result']['timeStamp']
