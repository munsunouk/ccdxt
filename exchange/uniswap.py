...
from ..base.utils.constants import MAX_UINT_128
...
class Uniswap(Exchange):
    ...
    async def remove_liquidity(self, tokenAsymbol, tokenBsymbol, amountSlippage, *args, **kwargs):
        ...
        tx = self.decrease_liquidity(
            self, pool["tokenId"], position, amountAMin, amountBMin, deadline
        )
    ...
    def burn(self, tokenId):
        ...
        tx = self.nft_managerContract.functions.burn(tokenId).build_transaction(
            {
                "from": self.account,
                "nonce": self.nonce,
            }
