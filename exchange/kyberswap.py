...
class Kyberswap(Exchange):
    ...
    async def fetch_ticker(self, amountAin, tokenAsymbol, tokenBsymbol, **kwargs):
        ...
        if self.proxy:
            quote_result = self.create_request(
                self.api_url, params, self.chains["baseChain"], "route", "encode", proxy=self.proxy
            )

        else:
            quote_result = self.create_request(
                self.api_url, params, self.chains["baseChain"], "route", "encode"
            )
    ...
    async def add_liquidity(
        ...
        **kwargs,
        ...
        nonFungiblePositionManagerAddress = self.set_checksum(
            self.markets["nonFungiblePositionManagerAddress"][int(isV3)]
        )

        self.factoryContract = self.get_contract(
            factoryAddress, self.markets["factoryAbi"][int(isV3)]
        )
        ...
        self.nft_managerContract = self.get_contract(
            nonFungiblePositionManagerAddress, self.markets["nonFungiblePositionManager"][int(isV3)]
        )
        ...
        self.require(
            amountA > tokenAalance["balance"], InsufficientBalance(tokenAalance, f"need :{amountA}")
        )
        self.require(
            amountB > tokenBalance["balance"], InsufficientBalance(tokenBalance, f"need :{amountB}")
        )
        ...
        self.require(
            not self.fetch_available_LBPairs(), ValueError("can`t find any available pairs")
        )
        ...
        self.check_approve(
            ...
            router=nonFungiblePositionManagerAddress,
        )
    ...
    def token_with_token(self, payload):
        tx = self.nft_managerContract.functions.mint(payload).build_transaction(
            {
                "from": self.account,
                "nonce": self.nonce,
            }
    ...
    def add_call_parameters(position, ticks, options):
        ...
        return {"calldata": multicall.encode_multicall(calldatas), value: Web3.toHex(value)}
