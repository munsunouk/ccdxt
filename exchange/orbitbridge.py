...
class Orbitbridge(Exchange):
    ...
    async def create_bridge(self, amount, from_tokenSymbol, to_tokenSymbol, fromChain, toChain, toAddr, *args, **kwargs):
        ...
        if 'additionalGasFee' in token :
            ...
            self.require(self.additionalGasFee >= baseCurrency["balance"], InsufficientBalance(baseCurrency, token['additionalGasFee']))
            self.require(token['MinimumBridge'] >= tokenBalance["balance"], InsufficientBalance(tokenBalance, token['MinimumBridge']))

            self.exp_additionalGasFee = self.from_value(value=self.additionalGasFee, exp=int(base_token["decimals"]))
        ...
        if from_tokenSymbol == 'kXRP' :
            ...
            tx = self._requestSwap(tokenAddress, toChain, xrp_deposit_address, amount, destinationTag)
    ...
    async def create_desTag(self, fromChain, *args, **kwargs) :
        ...
        try :
            ...
            result = int(self.create_request(self.api_url, {}, 'open', 'v1', 'api', 'xrp', 'tagRequest', fromChain, self.account)['tag'])
    ...
    def _requestSwap(
        self, tokenAddress: ChecksumAddress, toChain: str, toAddr: ChecksumAddress, amount: int, destinationTag : ChecksumAddress = None,
    ):
    ...
    def check_bridge_completed(self, tokenSymbol, toAddr, *args, **kwargs) :
        ...
        while True :
            ...
            if ((current_balance['balance'] - start_balance['balance']) >= ((self.amount - fee) / 2)) \
