...
class Klayswap(Exchange):
    ...
    async def fetch_ticker(self, amountAin, tokenAsymbol, tokenBsymbol) :
        ...
        amountin = await self.from_value(value = amountAin, exp = await self.decimals(tokenAsymbol))
        ...
        price_value = amountBout / await self.to_value(value = amountin, exp = await self.decimals(tokenAsymbol))
        
        price_amount = await self.to_value(value = price_value, exp = await self.decimals(tokenBsymbol))
        ...
        amountout = await self.to_value(value = amountB, exp = await self.decimals(tokenBsymbol))
