...
class Transaction(object) :
    ...
    async def fetch_transaction(self, tx, round = None):
        ...
        if tx_receipt["status"] != 1:
            ...
            return await self.fetch_trade_fail(tx_hash)
        
        if round == 'CHECK' :
            
            return print("approved token ")
        
        if self.chains['name'] == "KLAYTN" :
        
            if round == 'BRIDGE' :
                
                function, input_args = await self.routerContract.decode_function_input(self.get_transaction_data_field(tx))
                
                deposit = await self.routerContract.events.Deposit()
                
                events = await deposit.processReceipt(tx_receipt)
            ...
            if (round == 'SWAP') & (self.markets['name'] == 'Klayswap') :

                function, input_args = await self.routerContract.decode_function_input(self.get_transaction_data_field(tx))
                path = input_args["path"]
                
                swap = await self.routerContract.events.ExchangePos()
                
                events = await swap.processReceipt(tx_receipt)
        ...
        if self.chains['name'] == "MATIC" :
        
            if round == 'BRIDGE' :
                
                function, input_args = await self.routerContract.decode_function_input(self.get_transaction_data_field(tx))
                
                deposit = await self.routerContract.events.SwapRequest()
            ...
            if (round == 'SWAP') & (self.markets['name'] == 'Meshswap') :
    ...
    async def fetch_trade_fail(self,tx_hash) :
        ...
        try:
            result = await self.w3.eth.call(replay_tx)
        ...
        current_block_number = await self.w3.eth.block_number
